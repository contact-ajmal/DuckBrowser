...
try {
  ...
  const closed = await page.evaluate(async () => {
    ...
    return { before, after: qt.tabs.length, name: qt.active.name, persisted: JSON.parse(localStorage.getItem('duckbrowser.query.tabs')).tabs.length };
  });
  ...
  const ws = await page.evaluate(() => JSON.parse(localStorage.getItem('duckbrowser.settings')));
